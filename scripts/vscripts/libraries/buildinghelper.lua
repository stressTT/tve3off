...
function BuildingHelper:UpgradeBuilding(building, newName)
    ...
    if building:HasModifier("modifier_magnataur_skewer_slow") then
        local glyphStackModifier = building:FindModifierByName("modifier_magnataur_skewer_slow")
        if glyphStackModifier then
            local remainingTime = glyphStackModifier:GetRemainingTime()
            newBuilding:AddNewModifier(newBuilding, nil, "modifier_magnataur_skewer_slow",{duration = remainingTime})
        end
    end

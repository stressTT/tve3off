...
function InitializeTroll(hero)
    ...
    Timers:CreateTimer(function()
        ...
        if hero:IsAlive() then
            ...
            --local units = FindUnitsInRadius(DOTA_TEAM_GOODGUYS, caster:GetAbsOrigin() , nil, 900 , DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_HERO , DOTA_UNIT_TARGET_FLAG_NONE, 0 , false)
		    --for _,unit in pairs(units) do
			--    if unit ~= nil then
            ...
            if countAngel == 1 or countElf >= 3 then		--new
        ...
        if gameStartTimer > 0 then
            ...
            if GameRules.trollHero or GameRules.test then
                ...
                Notifications:BottomToAll(
                    ...
                    if GameRules.MapSpeed >= 4 and not string.match(GetMapName(),"clanwars") and not string.match(GetMapName(),"1x1") then

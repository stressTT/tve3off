...
function RevealArea( event )
	local status, nextCall = Error_debug.ErrorCheck(function() 
		...
		local visionRadius = string.match(GetMapName(),"1x1") and event.Radius*0.45 or string.match(GetMapName(),"arena") and event.Radius*0.58 or event.Radius
	...
	if parts ~= nil then      
		if  string.match(unit_name,"%a+") == "wisp" and parts["3"] == "normal" and unit_name ~= "gold_wisp" then
			if string.match(GetMapName(),"winter") then
				...
				elseif string.match(GetMapName(),"ghosttown") then 
				wearables:RemoveWearables(unit)
...
function BuyItem(event)
	...
	if hero:FindItemInInventory("item_havoc_hammer") ~= nil and item_name == 'item_disable_repair_2'  then
		SendErrorMessage(playerID, "error_full_inventory")
		ability:EndCooldown()
		return		
	end
...
function GlyphItem(keys)
	...
	--local tartar = targer:FindModifierByName("modifier_fountain_glyph")
	--if target:HasModifier("modifier_rooted") then
	--	tartar:SetDuration(1,true)
		--return
	--end

	--if target:HasModifier("modifier_buff_glyph_counter") then
	--	stack_count = target:GetModifierStackCount(modifier, ability)
	--	else 
	--	ability:ApplyDataDrivenModifier(caster, target, modifier, {})
	--end
	...
	--local tar = target:FindModifierByName( "modifier_fountain_glyph" ) --( "modifier_fountain_glyph" )
	--if target:HasModifier("modifier_buff_glyph_counter") and stack_count+1 == 1 then
		--target:AddNewModifier(target, target, "modifier_fountain_glyph", {Duration = time})
	--else
	--if target:HasModifier("modifier_buff_glyph_counter") and stack_count+1 == 2 then
	--	tar:SetDuration(5,true)
	--elseif target:HasModifier("modifier_buff_glyph_counter") and stack_count+1 == 3 then
	--	tar:SetDuration(3,true)
	--elseif target:HasModifier("modifier_buff_glyph_counter") and stack_count+1 > 3 then
	--	tar:SetDuration(1,true)
	--end
	...
	--FlagItem(keys)
	if  target:HasModifier("modifier_magnataur_skewer_slow") then
			target:AddNewModifier(target, target, "modifier_fountain_glyph", {Duration = timelocked})
			FlagItem(keys)
	elseif target then
		target:AddNewModifier(target, target, "modifier_fountain_glyph", {Duration = time})
		ability:ApplyDataDrivenModifier(caster, target, "modifier_magnataur_skewer_slow", {Duration = 120})
		FlagItem(keys)
	end

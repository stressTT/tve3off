...
function VoteKick(eventSourceIndex, event)
	...
	if votes[ event.playerID1 ] == 1 and team == DOTA_TEAM_BADGUYS 
		...
		then
			...
			message = "%s1 was rejected by Troll and will now help the elves as " .. GetModifiedName(ANGEL_HERO[i])
	...
	if team == DOTA_TEAM_GOODGUYS then
		Timers:CreateTimer(35.0, function() 
			...
			if (votes[ event.playerID1 ]/countVote[event.playerID1]) >= PERC_KICK_PLAYER + disKick and countVote[event.playerID1] >= MIN_PLAYER_KICK
        		...
        		message = "%s1 was rejected by elves and now help troll " ..
    			...
    			Timers:CreateTimer(timer, function()
        			...
        			UTIL_Remove(hero)
        			hero = PlayerResource:GetSelectedHeroEntity(playerID)
        			PlayerResource:SetCustomTeamAssignment(playerID, DOTA_TEAM_BADGUYS) -- A workaround for wolves sometimes getting stuck on elves team, I don't know why or how it happens.
        			FindClearSpaceForUnit(hero, pos, true)
    			end)
				CheckWolfInTeam(hero)
			end
